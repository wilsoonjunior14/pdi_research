...
class Database():
    ...
    def selectClouds(self, iteration):
        ...
        return self.connection.execute("SELECT id, x, y, width, height, pixels_by_area, pixels_by_perimeter, iteration_id, parent_id "+
    ...
    def createCloud(self, cloud) -> None:
        ...
        self.connection.execute("INSERT INTO cloud (x, y, width, height, pixels_by_area, pixels_by_perimeter, iteration_id, parent_id)"+
        "VALUES ("+str(cloud.x)+", "+str(cloud.y)+", "+str(cloud.width)+", "+str(cloud.height)+", "+str(cloud.amountPixelsArea)+", "+str(cloud.amountPixelsPerimeter)+", "+str(cloud.iterationId)+", "+str(cloud.parentId)+")");
        self.connection.commit()
    ...
    def initialize(self):
        ...
        self.connection.execute("CREATE TABLE cloud (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,"+
                                                    ...
                                                    "pixels_by_area INTEGER NOT NULL,"+
                                                    "pixels_by_perimeter INTEGER NOT NULL,"+
